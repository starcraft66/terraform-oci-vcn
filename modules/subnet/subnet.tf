...
resource "oci_core_subnet" "vcn_subnet" {
  ...
  ipv6cidr_blocks = var.enable_ipv6 ? lookup(each.value, "ipv6cidr_blocks", null) : null
  ...
  route_table_id = lookup(each.value, "type", "public") == "public" ? (
    ...
    lookup(each.value, "ipv6_nat_route", false) == true && var.ipv6_nat_route_id != null ?
    var.ipv6_nat_route_id : var.nat_route_id
  )
